...
@section('title', 'Repurchase Income')
@section('content')
            ...
            <div class="row mb-4">
                <div class="col-12">
                    <div class="page-title-box shadow-sm border-0">
                        <h4 class="mb-0 fs-20">
                            <i class="las la-sync-alt text-primary me-2"></i>REPURCHASE INCOME
                        </h4>
                    </div>
                </div>
            </div>
            ...
            <div class="card shadow-sm border-0">
                <div class="card-body p-0" style="background: #b9f6ca;">
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped table-hover align-middle mb-0" id="repurchaseIncomeTable">
...
function initializeTable() {
    dataTable = $('#repurchaseIncomeTable').DataTable({
        responsive: true,
        pageLength: 10,
        order: [[2, 'desc']],
        language: {
...
function loadData(dateFrom = '', dateTo = '') {
    ...
    const url = `{{ route('user.repurchase-income.data') }}?date_from=${dateFrom}&date_to=${dateTo}`;
            ...
            if (data.success) {
                ...
                (data.data || []).forEach((item, index) => {
                    ...
                    const fromUser = item.from_user
                        ? (item.from_user.user_name || item.from_user.first_name + ' ' + (item.from_user.last_name || ''))
                        : '—';
...
document.getElementById('repurchaseIncomeForm').addEventListener('submit', function(e) {
    e.preventDefault();
