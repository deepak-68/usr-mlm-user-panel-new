...
// Fetch WhatsApp number from API on page load
fetch('{{ env("API_BASE_URL") }}/whatsapp-number')
    .then(res => res.json())
    .then(data => {
        if (data.status && data.number) {
            const link = document.getElementById('whatsappLink');
            link.href = 'https://wa.me/' + data.number + '?text=Hi%20I%20need%20support';
        }
    })
    .catch(() => {});
</script>
